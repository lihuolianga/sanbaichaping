...
(async () => {
  ...
  const before = await editorPage.evaluate(() => {
    ...
    const date = document.querySelector("input[placeholder='请选择日期']");
    const time = document.querySelector("input[placeholder='请选择时间']");
    const card = document.querySelector(".publish-confirm-card");
    return {
      date: date ? date.value : "",
      time: time ? time.value : "",
      cardText: card ? (card.textContent || "").replace(/\s+/g, "").slice(0, 400) : "",
    };
  });
  console.log("=== 确认发布前 ===");
  console.log("日期:", before.date, "时间:", before.time);
  console.log("卡片文字:", before.cardText);
  ...
  const after = await editorPage.evaluate(() => {
    const isVis = el => { const r = el.getBoundingClientRect(); return r.width > 0 && r.height > 0; };
    const modals = Array.from(document.querySelectorAll(".byte-modal, .arco-modal, [role='dialog'], [class*='toast'], [class*='message'], [class*='notification'], [class*='alert']")).filter(isVis).map(m => (m.textContent || "").replace(/\s+/g, ""));
    const text = (document.body ? document.body.innerText : "").replace(/\s+/g, "");
    return { modals, fullBodyHead: text.slice(0, 500) };
  });
  console.log("=== 点确认发布后 ===");
  console.log("弹窗(完整):", JSON.stringify(after.modals, null, 1));
  console.log("页面文字前500:", after.fullBodyHead);

  await editorPage.screenshot({ path: path.join(__dirname, "diag_ch60.png"), fullPage: true });
  await ctx.close();
