...
// 案件8 需改名的3章（标题与番茄侧已发布章节重复）
const changes = [
  { no: "91", newTitle: "账册最后三页" },
  { no: "92", newTitle: "逆阵让老丁入髓" },
  { no: "95", newTitle: "复诊日期写进祠堂" },
];
...
(async () => {
  ...
  // 验证草稿箱标题（案件8 087-096 及 91/92/95 新标题）
  await wait(2000);
  ...
  const draftTab2 = page.locator("[role='tab'], [class*='tab']").filter({ hasText: /^草稿箱$/ }).first();
  if (await draftTab2.count().catch(() => 0)) { await draftTab2.click({ timeout: 2500 }).catch(() => {}); await wait(4000); }
