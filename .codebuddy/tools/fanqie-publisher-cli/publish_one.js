...
(async () => {
  ...
  await wait(3000);
  console.log(`第${chapterNo}章 定时到 ${dateStr} ${hour}:${minute}`);
  ...
  await wait(1000);
  console.log("日期:", JSON.stringify(await dateInput.inputValue().catch(() => "(读取失败)")));

  // 时间：点小时列 + 分钟列 + 确定
  const timeInput = editorPage.locator("input[placeholder='请选择时间']").first();
  await timeInput.click({ timeout: 2000 }).catch(() => {});
  await wait(1500);
  const hourList = editorPage.locator(".arco-timepicker-list").nth(0);
  const hourCell = hourList.locator(".arco-timepicker-cell").filter({ hasText: new RegExp("^" + hour + "$") }).first();
  if (await hourCell.count().catch(() => 0)) { await hourCell.click({ timeout: 2000 }).catch(() => {}); await wait(800); console.log("点击小时:", hour); }
  const minList = editorPage.locator(".arco-timepicker-list").nth(1);
  const minCell = minList.locator(".arco-timepicker-cell").filter({ hasText: new RegExp("^" + minute + "$") }).first();
  if (await minCell.count().catch(() => 0)) { await minCell.click({ timeout: 2000 }).catch(() => {}); await wait(800); console.log("点击分钟:", minute); }
  await clickBtn(editorPage, "确定");
  await wait(1000);
  console.log("时间:", JSON.stringify(await timeInput.inputValue().catch(() => "(读取失败)")));
  ...
  await editorPage.screenshot({ path: path.join(__dirname, "publish_one_before.png"), fullPage: true });
  ...
  const after = await editorPage.evaluate(() => {
    const isVis = el => { const r = el.getBoundingClientRect(); return r.width > 0 && r.height > 0; };
    const modals = [];
    Array.from(document.querySelectorAll(".byte-modal, .arco-modal, [role='dialog'], [class*='toast']")).forEach(m => {
      if (isVis(m)) modals.push((m.textContent || "").replace(/\s+/g, "").slice(0, 120));
    });
    const text = (document.body ? document.body.innerText : "").replace(/\s+/g, "");
    return { modals, hasSuccess: /发布成功|已发布|定时发布成功/.test(text) };
  });
  console.log("弹窗:", JSON.stringify(after.modals, null, 1));
  console.log("成功:", after.hasSuccess);
